...
class Inquiry
{
    ...
    public function getInquiriesBySupervisor($supervisor)
    {
        $query = "SELECT * FROM `inquiry` WHERE `property` IN (SELECT `id` FROM `property` WHERE `supervisor` = $supervisor) ORDER BY `id` DESC";
        $db = new Database();
        $result = $db->readQuery($query);
        $array_res = array();
        while ($row = mysql_fetch_array($result)) {
    ...
    public function sendMail($inquiry)
    {
        ...
        $SUPERVISOR = new Supervisor($PROPERTY->supervisor);
    ...
    public function sendMailToSupervisor($inquiry)
    {
        require_once "Mail.php";
        date_default_timezone_set('Asia/Colombo');
        $todayis = date("l, F j, Y, g:i a");
    ...
    <body>
                                                ...
                                                <tr> 
                                                    <td style="border:1px solid #dcdee3;padding:20px;background-color:#fff;width:600px" width="600px" bgcolor="#ffffff" align="center"> 
                                                        ...
                                                        <table style="background-color:#f5f7fa" width="100%" cellspacing="0" cellpadding="0" border="0" bgcolor="#F5F7FA"> 
                                                            <tbody> 
                                                                <tr> 
                                                                    <td style="font-size:14px;color:#333;line-height:18px;font-family:Arial,Helvetica,sans-serif;padding:15px 20px 10px;font-weight: 600;" align="left"> Hi , ' . $SUPERVISOR->name . ' </td> 
                                                                </tr> 
                                                            </tbody> 
                                                        </table> 
                                                        <table style="background-color:#f5f7fa" width="100%" cellspacing="0" cellpadding="0" border="0" bgcolor="#F5F7FA"> 
     ...
     <li>
        ...
        $companyMail = $smtp->send($SUPERVISOR->email, $companyHeaders, $company_message);
